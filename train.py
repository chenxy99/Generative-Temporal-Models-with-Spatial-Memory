...
state_dict = torch.load('saves/GTM_SM_state_dict_17.pth')
...
def train(epoch):
    ...
    for batch_idx, (data, _) in enumerate(loader_train):
        ...
        # transforming data
        training_data = Variable(data, requires_grad=False).type(dtype)
        ...
        for paras in GTM_SM_model.parameters():
            if torch.sum(torch.isnan(paras.grad.data)) > 0:
                print('error')
